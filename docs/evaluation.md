...
**`--mode generation`** (2026-09-16 and 2026-09-17, real Gemini `gemini-2.5-flash`, run across two calendar days in five passes — an initial `--limit 15` batch plus four `--mode generation --resume` passes, two per day): **8 of 50 cases have reached a real terminal outcome**; `overall_security_status = PENDING_REVIEW` and is being reported honestly as still pending, not forced to `passed` or `failed`, because no manual-review-designated case (`conflicting_evidence`/`prompt_injection`) has produced live content yet to actually review — see "Known limitations" below. Five cases (`dl-01`, `dl-02`, `dl-03`, `dl-04`, `cds-02`) received a live Gemini answer with zero security violations each. Four of the five scored fully correct (`status_correct = citation_correct = citation_complete = forbidden_absent = True`); `cds-02` is a genuine three-document cross-doc-synthesis success (dana, `initech-ltd`), correctly assembling the full `customer_expectation → product_approved → contractual` authority progression across `CALL-3001`, `CALL-3002`, and the `C-INITECH-INTERNAL` Slack message into one answer with distinct correct citations for each claim. **`dl-03` scored `citation_correct = False`** — classified as a `golden_data_problem`, not a product bug: the golden case's `expected_evidence` names only `C-GLOBEX-INTERNAL`, but bob (who is legitimately permitted to see both) received an answer that also correctly cited `C-GLOBEX-UPDATE`, the same "Globex timeline reversal" fixture referenced elsewhere in this milestone (Q4 target delayed to Q1) — genuinely relevant to the query "What does the internal Product note say about the Globex rate limit?" The answer is factually grounded and fully permission-safe (`forbidden_absent = True`, 0 security violations, `citation_complete = True`); the golden fixture's `expected_evidence` list is simply narrower than what's actually valid, mirroring a precedent Milestone 5 already documented and did not "fix" by editing product code or the dataset. The same discipline was followed here: no product change, no dataset edit made just to inflate a score. Three more cases (`pr-05`, `pr-07`, `pr-08`) reached `account_not_visible` — a structural short-circuit in `generation/service.py` that never calls the model, so these complete deterministically regardless of quota. The remaining 42 cases all received an explicit `429 RESOURCE_EXHAUSTED` naming `GenerateRequestsPerDayPerProjectPerModel-FreeTier, limit: 20`, confirmed via direct, minimal API probes outside the evaluation runner on both days (not inferred from the runner's own error handling) — this is quota, not a bug, and per this milestone's own design brief the architecture was **not** changed in response. Every blocked attempt was correctly recorded as `generation_failure` with **zero security-gate violations**, and `--resume`'s skip/retry logic was re-verified against ground truth in the persisted `.eval-output/generation-latest.json` on the second day: exactly the cases already completed were skipped on each successive `--resume` pass, and exactly the incomplete cases were retried. The observed refill pattern across the two days was a small, effectively-fixed daily allotment (roughly 1–2 additional live calls became available per day, consumed by whichever case was first in line, then immediately exhausted again for the rest of that day, confirmed by repeated immediate-retry probes failing identically) rather than a continuous per-minute trickle — at this rate, exhausting the free tier honestly to complete all 42 remaining cases would take many more days, not one more session. **Still incomplete, carried forward from Milestone 5**: the dedicated live prompt-injection case (all 4 `prompt_injection` cases still await a live answer) and one live end-to-end `POST /answer` HTTP round trip (still not attempted — quota was exhausted both times this was attempted). Every `unauthorized_facts_emitted`/`hidden_conflict_leakage` manual review remains open for the same reason: there is no designated-category live content yet to review. All of this remains resumable via `--mode generation --resume` without re-spending any completed call, across as many future quota windows as it takes.
...
**Fourth pass (real-Gemini generation baseline, 2026-09-16/17)**: one real-generation case (`dl-03`) scored `citation_correct = False`. Classified as a `golden_data_problem`, not a product bug — see "Baseline results" above for the full explanation (the golden fixture's `expected_evidence` under-names what's actually valid, permitted evidence; the model's answer is correct and grounded). No pytest regression, product fix, or dataset edit was made, matching this milestone's "measure → classify → fix only if justified" discipline: a golden-data precision gap does not justify touching `retrieval/`, `generation/`, or the fixture just to make a score look better.
...
- **The real-Gemini baseline is incomplete** (8 of 50 cases reached a real terminal outcome as of 2026-09-17; 5 with a live model answer, 3 structurally without one) — a quota constraint, not a design gap; see "Baseline results" above. The live prompt-injection case and one live end-to-end `POST /answer` round trip (both carried forward from Milestone 5) remain untried for the same reason. Observed refill behavior across two calendar days suggests roughly 1–2 additional live calls become available per day, not a fast trickle — completing all 42 remaining cases on the free tier would take many more days of `--resume`, not one more session; a paid tier is the practical way to actually finish this baseline.
- **`overall_security_status` for the real-generation mode is reported as `pending_review`, not forced to `passed` or `failed`**, because the two manual-adjudication gates (`unauthorized_facts_emitted`, `hidden_conflict_leakage`) cannot be honestly adjudicated without live content to review, and no designated `conflicting_evidence`/`prompt_injection` case has produced any yet (the 5 live answers obtained so far were all `direct_lookup`/`cross_doc_synthesis` cases). A completed report must not claim these at zero, and must not claim an overall `passed`/`failed` verdict, until real reviews exist on real designated-category output.
